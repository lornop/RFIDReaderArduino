...
//This my function to get just the raw data from the UID of the card. It was copied from above and modified. All credit to the OG authors
...
//Function to send the UID to a string. Use this before transmitting so we can get a checksum and stufff	
	String MFRC522::PICC_DumpUIDToString(Uid *uid)	///< Pointer to Uid struct returned from a successful PICC_Select().
	{
	String OutputString;
	for (byte i = 0; i < uid->size; i++) 
	    {
	    ...
	    {
		    OutputString += "0";
		    OutputString += String((uid->uidByte[i]), HEX);
		}
		else
		    OutputString += String((uid->uidByte[i]), HEX);
	    }
